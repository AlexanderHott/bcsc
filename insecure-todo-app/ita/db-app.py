...
@get("/search")
async def search_for_item(state: State, title: str | None = None) -> TodoCollectionType:
    async with sessionmaker(bind=state.engine) as session:
        if title is not None:
            todos = await session.execute(text(f"SELECT title, done FROM todo_items WHERE title LIKE '%{title}%';"))
            return [{"title": t[0], "done": bool(t[1])} for t in todos]
...
app = Litestar([get_list, add_item, update_item, search_for_item], lifespan=[db_connection])
